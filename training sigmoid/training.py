...
# function does one backwards pass
...
def trainingData(inputNodes,hidLay,outNode,learningParameter,interval,x_trainNumpy, y_trainNumpy,epochs):
    ...
    for j in range(0,interval):
        ...
        #loop torugh each row in training data
...
def main():
    ...
    # .iloc[:,1:] removes the first column (row index numbers) as I didnt need them.
    ...
    noHiddenNodes = 2
    # generate nodes before calling the algorithm
    hidLay, outNode, inputNodes = generate_hidden_weight_bias(noHiddenNodes, x_trainNumpy, y_trainNumpy, outPutrow=0)
    # learning parameter stays constant (apart from annealing)
    learningParameter = 0.1
    epochs = 2000
    # run the algorithm using the geenrated nodes as inputs
    ...
    # convert the list of mse to a numpy array then to a dataframe
    mseListNp = np.array(mseList)
    mse_df = pd.DataFrame(mseListNp)
    # wrrite the dataframe to an excel file
    mse_df.to_excel("training" + str(len(hidLay))+"hid" + "_" + str(epochs) +"epo"+ "_.xlsx")
